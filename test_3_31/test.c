...
double  双精度浮点型
int main()
{//sizeof计算变量类型的大小，而%zu更加合适用来表示变量类型的大小
	printf("%zu\n", sizeof(char));
	printf("%zu\n", sizeof(short));
...
int main()
{
	...
	printf("%d", a);
	return 0;
}//而这段代码，变量a也只能在其范围内使用，但是范围明显大了许多
 2.全局变量
 //全局变量的作用域是整个工程
 extern int a; //声明来自外部符号(声明了scanf添加测试.c文件中的整型变量a)
int main()
{
	{
		printf("%d", a);
...
2.全局变量的生命周期
 //全局变量可以说是整个程序的生命周期（这是一段代码，不妨可以试试）
 int a = 10;
int main()
{
	{
